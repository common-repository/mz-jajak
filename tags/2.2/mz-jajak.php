...
?>
			...
			<p><label for="mzjajak_bgcolor"><?php _e('Pie Chart Background Color:'); ?><br/><input	style="width: 100px;" id="mzjajak_bgcolor" name="mzjajak_bgcolor" maxlength="6" type="text" class="colorwell" value="<?php echo $mzjajak_bgcolor; ?>" /></label><div id='colorpicker'></div></p>
			<p><label for="mzjajak_fcolor"><?php _e('Pie Chart Font Color:'); ?><br/><input	style="width: 100px;" id="mzjajak_fcolor" name="mzjajak_fcolor" maxlength="6" type="text"  class="colorwell" value="<?php echo $mzjajak_fcolor; ?>" /></label></p>
...
function show_jajak($args) {
	...
	if (($_COOKIE['mzjajak']=="submitted") or ($_POST['formvote']=="Y")) {
		if (!empty($_POST['answer']) && ($_COOKIE["mzjajak"]!="submitted")) {
			$answer=$_POST['answer'];
			$id=$_POST['id'];
			switch ($answer) {
				case 1:
			...
			echo "<td valign=bottom align='center'><img src='".get_bloginfo('url')."/wp-content/plugins/mz-jajak/include/img.php' alt='Result' width='100%' /></td></tr>";
			echo '<tr><td style="font-size:x-small" align="right">Created by <a href="http://www.tirta-fajri.net">Mizi</a></td><tr></table>' . $after_widget;
	} else { 
	$rows = $wpdb->get_results("SELECT * FROM " . $table_name . " WHERE enabled=1");
		...
		foreach ($rows as $rows) {
			...
			echo "<br /><br /><table align='center' width='100%' cellspacing='5'>";
			if (!$rows->name1=="") { echo '<tr><td width="4">' . '<input type="radio" name="answer" value="1" /> '.'</td><td>'.stripslashes($rows->name1)."</td></tr>"; 		}
			if (!$rows->name2=="") { echo '<tr><td width="4">' . '<input type="radio" name="answer" value="2" /> '.'</td><td>'.stripslashes($rows->name2)."</td></tr>"; }
			if (!$rows->name3=="") { echo '<tr><td width="4">'. '<input type="radio" name="answer" value="3" /> '.'</td><td>'.stripslashes($rows->name3)."</td></tr>"; }
			if (!$rows->name4=="") { echo '<tr><td width="4">' . '<input type="radio" name="answer" value="4" /> '.'</td><td>'.stripslashes($rows->name4)."</td></tr>"; }
			if (!$rows->name5=="") { echo '<tr><td width="4">' . '<input type="radio" name="answer" value="5" /> '.'</td><td>'.stripslashes($rows->name5)."</td></tr>"; }
			if (!$rows->name6=="") { echo '<tr><td width="4">' . '<input type="radio" name="answer" value="6" /> '.'</td><td>'.stripslashes($rows->name6)."</td></tr>"; }
			echo "<tr><td colspan=2></td></tr>";
			echo '<tr><td colspan=2><input type="hidden" name="formvote" value="Y" /><input type="hidden" name="id" value="'.$rows->id.'" /><input type="image" value="Submit" src="'.get_bloginfo('url').'/wp-content/plugins/mz-jajak/include/vote.png" alt="Vote" /></td></tr>';
			echo '<tr><td style="font-size:x-small" align=right colspan=2>Created by <a href="http://www.tirta-fajri.net">Mizi</a></td><tr></table></form>' . $after_widget;
		}
	}	
}
